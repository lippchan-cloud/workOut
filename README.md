...
6. 选中日期后，下面有列表，按照时间正序展示（含时分），消耗绿色，摄入红色。可以按当前筛选「导出」（实际 xlsx：事项列表 + 成长曲线两工作表）或「分享」H5；须先填身高和体重。
7. 我的：先三个选项（身体资料 / 账号安全 / 退出登录）；身体资料含真实日期，页下方是成长曲线（单位 cm/kg，可拖、放大改时间粒度）；改资料会留历史；支持退出登录。公开报告页 `/report/:id` 不走三 Tab。
