...
The Profile tab SHALL NOT place body fields, password change, and account deletion on the same screen. `/profile` MUST be a level-2 option list. 身体资料 MUST live at `/profile/body` (nickname, height, weight, save). 账号安全 MUST live at `/profile/account` (change password, delete account). Saving body data MUST NOT submit password fields. Change-password controls MUST remain available on the account page.
